...
if (formTambahManga) {
    formTambahManga.addEventListener('submit', async (e) => {
        ...
        try {
            const response = await axios.post('http://0.0.0.0:3000/manga/create', {
                Title,
                Chapter,
                Status,
                ReleaseDate,
...
const fetchManga = async () => {
    try {
    const response = await axios.get('http://0.0.0.0:3000/manga');
    ...
    mangaData.forEach((manga) => {
        ...
        row.innerHTML = `
                ...
                <td>
                    <button class="btn btn-warning btn-sm" onclick="editManga('${manga.id}')">Edit</button>
                    <button class="btn btn-danger btn-sm" onclick="deleteManga('${manga.id}')">Delete</button>
                </td>
            `;
